// IV — melhorias personalizadas: telefone do aluno, relatórios filtrados e modal premium
(function(){
  'use strict';
  ...
  function getMODULOS(){
    try { return (typeof MODULOS !== 'undefined') ? MODULOS : null; } catch(e){ return null; }
  }
  ...
  function ensureStyles(){
    if(document.getElementById('iv-custom-improvements-style')) return;
    var css = document.createElement('style');
    css.id = 'iv-custom-improvements-style';
    css.textContent = `
      .iv-phone-cell{white-space:nowrap;color:#BFEAFF;font-size:12px;}
      .iv-phone-cell a{color:#BFEAFF;text-decoration:none;font-weight:800;}
      .iv-phone-cell a:hover{color:#FFFFFF;text-decoration:underline;}
      .iv-phone-input-hint{display:block;margin-top:4px;color:#6B8AB0;font-size:10px;font-weight:600;letter-spacing:.02em;}
      .iv-link-actions-premium{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:10px!important;justify-content:stretch!important;align-items:stretch!important;margin-top:14px!important;}
      .iv-link-actions-premium .btn{width:100%!important;min-height:44px!important;justify-content:center!important;border-radius:999px!important;font-size:12px!important;font-weight:900!important;white-space:nowrap!important;letter-spacing:.01em!important;box-shadow:0 14px 30px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.10)!important;}
      .iv-link-actions-premium .btn:hover{transform:translateY(-2px)!important;box-shadow:0 18px 40px rgba(0,0,0,.36),0 0 22px rgba(126,200,240,.16),inset 0 1px 0 rgba(255,255,255,.14)!important;}
      .iv-link-actions-premium .iv-btn-whats{background:linear-gradient(135deg,#25D366,#7EDBA8)!important;color:#02120A!important;border:0!important;}
      .iv-link-actions-premium .iv-btn-open{background:linear-gradient(135deg,#2F80ED,#22D3EE)!important;color:#fff!important;border:0!important;}
      .iv-link-actions-premium .iv-btn-copy{background:linear-gradient(135deg,rgba(126,200,240,.16),rgba(47,128,237,.10))!important;color:#DFF6FF!important;border:1px solid rgba(126,200,240,.28)!important;}
      .iv-link-actions-premium .iv-btn-close{background:rgba(255,255,255,.035)!important;color:#B9CBE2!important;border:1px solid rgba(126,200,240,.16)!important;}
      .iv-missed-list{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;}
      .iv-missed-chip{display:inline-flex;align-items:center;gap:4px;padding:5px 8px;border-radius:999px;background:rgba(224,85,85,.12);border:1px solid rgba(224,85,85,.25);color:#F6A2A2;font-size:10px;font-weight:800;}
      .iv-ok-chip{display:inline-flex;align-items:center;gap:4px;padding:5px 8px;border-radius:999px;background:rgba(62,201,122,.12);border:1px solid rgba(62,201,122,.25);color:#9EF0BC;font-size:10px;font-weight:800;}
      @media(max-width:820px){.iv-link-actions-premium{grid-template-columns:repeat(2,minmax(0,1fr))!important}.iv-phone-cell{font-size:11px}}
      @media(max-width:430px){.iv-link-actions-premium{grid-template-columns:1fr!important}.iv-link-actions-premium .btn{min-height:42px!important}}
    `;
  ...
  function ensureTelefoneField(){
    ...
    if(document.getElementById('al-telefone')) return document.getElementById('al-telefone');

    var row = inscricao.closest('.form-row');
    var wrapInscricao = inscricao.parentElement;
    if(row) row.classList.add('tri');

    var box = document.createElement('div');
    box.innerHTML = '<label>Celular/Telefone <span style="color:var(--muted);font-size:10px;font-weight:600">(opcional)</span></label>' +
      '<input id="al-telefone" type="tel" inputmode="tel" autocomplete="tel" maxlength="15" placeholder="(11) 99999-9999">' +
      '<span class="iv-phone-input-hint">Formatação automática durante a digitação.</span>';

    if(wrapInscricao && wrapInscricao.parentElement){
      wrapInscricao.insertAdjacentElement('afterend', box);
    }

    var input = document.getElementById('al-telefone');
    if(input && !input.dataset.ivPhoneMask){
      input.dataset.ivPhoneMask = '1';
  ...
  function normalizarTelefoneDB(){
    var db = getDB();
    if(!db || !Array.isArray(db.alunos)) return;
    db.alunos.forEach(function(a){
      if(a && typeof a.telefone === 'undefined') a.telefone = '';
      if(a && a.telefone) a.telefone = formatarTelefone(a.telefone);
    });
  }

  function wrapGlobal(nome, factory){
    var old = window[nome];
    if(typeof old !== 'function' || old._ivCustom) return false;
    var novo = factory(old);
    novo._ivCustom = true;
    window[nome] = novo;
    return true;
  }

  function patchAlunoModal(){
    ensureTelefoneField();
    normalizarTelefoneDB();

    wrapGlobal('normalizarDB', function(old){
      return function(){
    ...
    wrapGlobal('openAlunoModal', function(old){
      return function(id){
        var r = old.apply(this, arguments);
        var input = ensureTelefoneField();
        var db = getDB();
        if(input){
          if(id && db && Array.isArray(db.alunos)){
            var al = db.alunos.find(function(a){ return a.id == id; });
            input.value = formatarTelefone(al && al.telefone || '');
          } else {
            input.value = '';
          }
        }
    ...
    wrapGlobal('salvarAluno', function(old){
      return function(){
        var input = ensureTelefoneField();
        var telefone = formatarTelefone(input && input.value || '');
        var idVal = (document.getElementById('al-id') || {}).value || '';
        var nome = ((document.getElementById('al-nome') || {}).value || '').trim();
        if(input) input.value = telefone;
        var r = old.apply(this, arguments);
        setTimeout(function(){
          var db2 = getDB();
          if(!db2 || !Array.isArray(db2.alunos) || !nome) return;
          var aluno = idVal ? db2.alunos.find(function(a){ return a.id == idVal; }) : db2.alunos.slice().reverse().find(function(a){ return (a.nome || '').trim() === nome; });
          if(!aluno) return;
          if((aluno.telefone || '') !== telefone){
  ...
  function listaAlunosVisivel(){
    var db = getDB();
    if(!db || !Array.isArray(db.alunos)) return [];
    var busca = ((document.getElementById('busca-aluno') || {}).value || '').toLowerCase();
    var fEq = (document.getElementById('filtro-equipe') || {}).value;
    var fTurma = (document.getElementById('filtro-turma') || {}).value;
    var lista = db.alunos.slice().sort(function(a,b){ return (a.nome || '').localeCompare(b.nome || '', 'pt-BR'); });
    if(busca) lista = lista.filter(function(a){
      return (a.nome || '').toLowerCase().includes(busca) ||
             (a.inscricao || '').toString().toLowerCase().includes(busca) ||
             (a.telefone || '').toString().toLowerCase().includes(busca) ||
             digitosTelefone(a.telefone || '').includes(digitosTelefone(busca));
    });
    if(fEq) lista = lista.filter(function(a){ return a.equipeId == fEq; });
    if(fTurma) lista = lista.filter(function(a){ return (a.turma || '') == fTurma; });
    return lista;
  }

  function patchTabelaAlunosTelefone(){
    ...
    if(headerRow && !headerRow.querySelector('[data-iv-phone-th]')){
      var ths = headerRow.children;
      var th = document.createElement('th');
      th.setAttribute('data-iv-phone-th', '1');
      th.textContent = 'Telefone';
      headerRow.insertBefore(th, ths[3] || null);
    }

    if(!tbody.children.length) return;
    var primeiraLinha = tbody.children[0];
    if(primeiraLinha && primeiraLinha.children.length === 1){
      primeiraLinha.children[0].setAttribute('colspan','10');
      return;
    }

    var lista = listaAlunosVisivel();
    Array.prototype.forEach.call(tbody.children, function(tr, i){
      if(tr.querySelector('.iv-phone-cell')) return;
      var al = lista[i];
      var td = document.createElement('td');
      td.className = 'iv-phone-cell';
      var tel = formatarTelefone(al && al.telefone || '');
      var digits = digitosTelefone(tel);
      td.innerHTML = tel ? '<a href="tel:' + esc(digits) + '">' + esc(tel) + '</a>' : '<span style="color:var(--muted)">—</span>';
      tr.insertBefore(td, tr.children[3] || null);
    });
  }

  function patchRenderAlunos(){
    wrapGlobal('renderAlunos', function(old){
      return function(){
        var r = old.apply(this, arguments);
        normalizarTelefoneDB();
        setTimeout(patchTabelaAlunosTelefone, 0);
        return r;
      };
    });
    patchTabelaAlunosTelefone();
  }
  ...
  function rotuloAula(m, si, au){
    var mods = getMODULOS();
    var mod = mods && mods[m];
    var sem = mod && mod.semanas && mod.semanas[si];
    var nomeAula = au === 'INAU' ? 'Aula Inaugural' : au === 'AP' ? 'Apresentação' : 'Aula ' + String(au).padStart(2,'0');
    var semLabel = sem && sem.label ? sem.label.replace(/\s+/g,' ').trim() : 'Semana ' + (Number(si) + 1);
    return (mod && mod.nome || (m + 'º Módulo')) + ' · ' + semLabel + ' · ' + nomeAula;
  }
  function statsAlunoModulo(al, m, registradas){
    ...
    mod.semanas.forEach(function(sem, si){
      (sem.aulas || []).forEach(function(au){
        ...
        total++;
        var presente = !!db.presencas[presKey(al.id, m, si, au)];
        if(presente) presencas++;
        else perdidas.push(rotuloAula(m, si, au));
      });
    });
  ...
  function chipsAulasPerdidas(perdidas, limite){
    if(!perdidas || !perdidas.length) return '<div class="iv-missed-list"><span class="iv-ok-chip">✅ Nenhuma aula perdida registrada</span></div>';
    var max = limite || 50;
    var html = perdidas.slice(0, max).map(function(a){ return '<span class="iv-missed-chip">❌ ' + esc(a) + '</span>'; }).join('');
    if(perdidas.length > max) html += '<span class="iv-missed-chip">+' + (perdidas.length - max) + ' aulas</span>';
    return '<div class="iv-missed-list">' + html + '</div>';
  }
  function gerarHTMLRelatorioMelhorado(tipo, modFiltro, titulo, versao){
    ...
    if(tipo === 'caminho') body += `<div class="history-grid">${rows.map(function(a){ var steps = a.stats.map(function(s){ var nomeMod = (getMODULOS() && getMODULOS()[s.modulo] && getMODULOS()[s.modulo].nome) || (s.modulo + 'º Módulo'); return `<div class="step"><b>${esc(nomeMod)}</b><div class="meta">Aulas registradas: ${s.total}</div><div style="margin-top:8px"><span class="tag g">${s.presencas} pres.</span> <span class="tag ${s.faltas ? 'r' : 'g'}">${s.faltas} faltas</span> <span class="tag b">${s.pct}%</span></div></div>`; }).join(''); return `<div class="card"><div class="history-head"><div><div class="history-name">${esc(a.nome)}</div><div class="meta">${esc(a.equipe)} · ${esc(a.turma)} · ${esc(a.mod)}${a.telefone ? ' · ' + esc(a.telefone) : ''}</div></div><span class="tag ${a.sit === 'DESISTENTE' ? 'r' : 'g'}">${esc(a.sit)}</span></div><div class="route">${steps}</div><div class="st" style="font-size:13px;margin-top:14px;margin-bottom:4px;color:#F08080">Aulas que o aluno perdeu</div>${chipsAulasPerdidas(a.perdidas, 80)}</div>`; }).join('')}</div>`;
    ...
    return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(titulo)} — ${esc(tipoNome)}</title>${css}</head><body><main class="shell"><header class="hero"><div class="badge">${esc(tipoNome)} · ${esc(filtroNome)} · Versão ${versao === 'web' ? 'Web' : 'Mobile'}</div><div class="title">${esc(titulo)}</div><div class="sub">Gerado em ${dt} · Instituto de Vencedores</div></header><section class="content">${body}</section></main></body></html>`;
  }
  ...
  function premiumizeModal(){
    ...
    Array.prototype.forEach.call(actions.querySelectorAll('button'), function(btn){
      ...
      if(txt.indexOf('whatsapp') >= 0) btn.classList.add('iv-btn-whats');
      else if(txt.indexOf('abrir link') >= 0 || txt.indexOf('abrir o link') >= 0) btn.classList.add('iv-btn-open');
      else if(txt.indexOf('copiar') >= 0) btn.classList.add('iv-btn-copy');
      else if(txt.indexOf('fechar') >= 0) btn.classList.add('iv-btn-close');
    });
  }
  function patchRelatorio(){
    var old = window.gerarRelatorioLink;
    if(typeof old === 'function' && !old._ivCustomReport){
      window.gerarRelatorioLink = async function(){
        ...
        if(tipo === 'visao'){
          ...
          else html = gerarHTMLRelatorioMelhorado('alunos', modFiltro, titulo, versao);
        } else html = gerarHTMLRelatorioMelhorado(tipo, modFiltro, titulo, versao);
  ...
  function patchAll(){ ensureStyles(); patchAlunoModal(); patchRenderAlunos(); patchRelatorio(); }
  ready(function(){ patchAll(); setTimeout(patchAll, 300); setTimeout(patchAll, 900); setTimeout(patchAll, 1800); });
})();
