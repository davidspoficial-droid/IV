...
(function(){
  if(window.__IV_PERMISSIONS_SPLIT_FIXED__) return;
  window.__IV_PERMISSIONS_SPLIT_FIXED__ = true;
  ...
  function instalar(){
    ...
    try{
      if(typeof setPermissoesForm==='function' && !setPermissoesForm.__splitOk){
        var oldSet=setPermissoesForm;
        var novoSet=function(perms){ajustarTela();return oldSet(expandir(perms));};
        novoSet.__splitOk=true;
        window.setPermissoesForm=setPermissoesForm=novoSet;
      }
    }catch(err){}

    try{
      if(typeof permissoesSelecionadas==='function' && !permissoesSelecionadas.__splitOk){
        var oldGet=permissoesSelecionadas;
        var novoGet=function(){var p=oldGet();delete p.alunos_manage;delete p.equipes_manage;return p;};
        novoGet.__splitOk=true;
