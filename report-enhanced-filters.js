import('./iv-pwa-runtime-fix.js?v=20260716-2')
  .then(function(){ return import('./iv-firebase-concurrency-guard.js?v=20260716-1'); })
  .then(function(){ return import('./iv-permission-authority.js?v=20260716-1'); })
  .then(function(){ return import('./iv-permission-menu-stability.js?v=20260716-1'); })
  .then(function(){ return import('./iv-permission-entry-recovery.js?v=20260717-2'); })
  .then(function(){ return import('./iv-mobile-menu-stay-open.js?v=20260717-2'); })
  .then(function(){ return import('./iv-presence-bulk-permission.js?v=20260716-1'); })
  .then(function(){ return import('./iv-revision-core.js?v=20260714-2'); })
  .then(function(){ return import('./iv-revision-year-min-2026.js?v=20260714-1'); })
  .then(function(){ return import('./iv-report-week-selection-v7.js?v=20260714-1'); })
  .then(function(){ return import('./iv-report-module-header-v8.js?v=20260714-1'); })
  .then(function(){ return import('./iv-report-postprocess-v5.js?v=20260714-2'); })
  .then(function(){ return import('./iv-report-general-authority-v4.js?v=20260714-1'); })
  .then(function(){ window.IVGenerateGeneralViewV4 = window.IVGenerateGeneralView; return import('./iv-reports-generator-v2.js?v=20260714-1'); })
  .then(function(){ return import('./iv-reports-general-view-v3.js?v=20260714-1'); })
  .then(function(){ return import('./iv-app-unified.js?v=20260715-1'); })
  .then(function(){ return import('./iv-navigation-speed-colors.js?v=20260716-2'); })
  .then(function(){ return import('./iv-mobile-menu-singleton.js?v=20260717-2'); })
  .then(function(){ return import('./iv-mobile-users-history-premium.js?v=20260716-1'); })
  .then(function(){ return import('./iv-mobile-users-web-guard.js?v=20260716-1'); })
  .then(function(){ return import('./iv-mobile-users-action-index-fix.js?v=20260716-1'); })
  .then(function(){ return import('./iv-student-module-status-filters.js?v=20260715-1'); })
  .then(function(){ return import('./iv-student-table-authority.js?v=20260715-1'); })
  .then(function(){
    if(window.IVGenerateGeneralViewV4) window.IVGenerateGeneralView = window.IVGenerateGeneralViewV4;
    if(window.IVPermissionAuthority) window.IVPermissionAuthority.refresh();
    if(window.IVPermissionMenuStability) window.IVPermissionMenuStability.refresh();
  })
  .catch(function(error){ console.error('Erro ao iniciar aplicação unificada do IV', error); });